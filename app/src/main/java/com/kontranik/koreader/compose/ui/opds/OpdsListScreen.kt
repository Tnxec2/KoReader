...
fun OpdsListScreen(
    ...
    navigateBack: () -> Unit,
    openOpdsEntryDetails: (entry: Entry, startUlr: String) -> Unit,
        ...
        onClick = { entry ->
            ...
            if (entry.clickLink != null) {
                entry.clickLink.href?.let {
                    if (it == "back") opdsViewModell.goBack() else opdsViewModell.load(it)
                }
            } else {
                openOpdsEntryDetails(entry, opdsViewModell.startUrl.value)
            }
        },
        onDelete = { pos ->
