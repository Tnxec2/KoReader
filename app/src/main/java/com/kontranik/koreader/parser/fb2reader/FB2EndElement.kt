...
object FB2EndElement {
    ...
    private fun endElementBinary(`object`: FB2ParserObject) {
        if (!`object`.onlyscheme || `object`.fb2scheme.cover == null && `object`.myBinaryData != null && `object`.fb2scheme.description.titleInfo.coverImageSrc == `object`.myBinaryData!!.name) {
    ...
    private fun parseEndElementInSection(fel: FB2Elements, fB2ParserObject: FB2ParserObject) {
        if (fel == FB2Elements.TITLE) {
            if (fB2ParserObject.mySection != null && fB2ParserObject.mySection!!.title == null) {
                fB2ParserObject.mySection!!.title = fB2ParserObject.myText.toString().trim { it <= ' ' }
            }
            fB2ParserObject.clearMyText()
        }
        if (!fB2ParserObject.isSection && !fB2ParserObject.isAnnotation && !fB2ParserObject.isCoverpage && !fB2ParserObject.isHistory) return
        ...
        when (fel) {
            ...
            FB2Elements.P, FB2Elements.STANZA -> result = if (!fB2ParserObject.isTitle) "</p>" else "<br/>"
            ...
            else -> Logger.getLogger("FB2ENDELEMENT").log(Level.INFO, "Other EndElement: " + fel.elName)
        }
        if (fB2ParserObject.isAnnotation) {
            ...
            Logger.getLogger("FB2ENDELEMENT")
                    .log(Level.INFO, "FEL: " + fel.elName + ", Other FB2ParserObject: " + fB2ParserObject.toString())
        }
    }
    ...
    private fun endElementDescription(el: FB2Elements, `object`: FB2ParserObject) {
        ...
        when (el) {
            ...
            FB2Elements.BOOKTITLE -> {
                if (`object`.isDescriptionTitleInfo) `object`.fb2scheme.description.titleInfo.booktitle = `object`.myText.toString().trim { it <= ' ' }
                `object`.clearMyText()
            }
            FB2Elements.FIRSTNAME -> {
                if (`object`.isDescriptionTitleInfo) {
                    if (`object`.isAuthor) {
                        ...
                        `object`.fb2scheme.description.titleInfo.authors[aIndex].firstname = `object`.myText.toString().trim { it <= ' ' }
                    } else if (`object`.isTranslator) {
                        aIndex = `object`.fb2scheme.description.titleInfo.translators.size - 1
                        `object`.fb2scheme.description.titleInfo.translators[aIndex].firstname = `object`.myText.toString().trim { it <= ' ' }
                    }
                } else if (`object`.isDescriptionDocumentInfo) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.documentInfo.authors.size - 1
                        `object`.fb2scheme.description.documentInfo.authors[aIndex].firstname = `object`.myText.toString().trim { it <= ' ' }
                    }
                }
                `object`.clearMyText()
            }
            FB2Elements.MIDDLENAME -> {
                if (`object`.isDescriptionTitleInfo && `object`.isAuthor) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.titleInfo.authors.size - 1
                        `object`.fb2scheme.description.titleInfo.authors[aIndex].middlename = `object`.myText.toString().trim { it <= ' ' }
                    } else if (`object`.isTranslator) {
                        aIndex = `object`.fb2scheme.description.titleInfo.translators.size - 1
                        `object`.fb2scheme.description.titleInfo.translators[aIndex].middlename = `object`.myText.toString().trim { it <= ' ' }
                    }
                } else if (`object`.isDescriptionDocumentInfo) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.documentInfo.authors.size - 1
                        `object`.fb2scheme.description.documentInfo.authors[aIndex].middlename = `object`.myText.toString().trim { it <= ' ' }
                    }
                }
                `object`.clearMyText()
            }
            FB2Elements.LASTNAME -> {
                if (`object`.isDescriptionTitleInfo && `object`.isAuthor) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.titleInfo.authors.size - 1
                        `object`.fb2scheme.description.titleInfo.authors[aIndex].lastname = `object`.myText.toString().trim { it <= ' ' }
                    } else if (`object`.isTranslator) {
                        aIndex = `object`.fb2scheme.description.titleInfo.translators.size - 1
                        `object`.fb2scheme.description.titleInfo.translators[aIndex].lastname = `object`.myText.toString().trim { it <= ' ' }
                    }
                } else if (`object`.isDescriptionDocumentInfo) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.documentInfo.authors.size - 1
            ...
            FB2Elements.NICKNAME -> {
                if (`object`.isDescriptionTitleInfo && `object`.isAuthor) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.titleInfo.authors.size - 1
                        `object`.fb2scheme.description.titleInfo.authors[aIndex].nickname = `object`.myText.toString().trim { it <= ' ' }
                    } else if (`object`.isTranslator) {
                        aIndex = `object`.fb2scheme.description.titleInfo.translators.size - 1
                        `object`.fb2scheme.description.titleInfo.translators[aIndex].nickname = `object`.myText.toString().trim { it <= ' ' }
                    }
                } else if (`object`.isDescriptionDocumentInfo) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.documentInfo.authors.size - 1
                        `object`.fb2scheme.description.documentInfo.authors[aIndex].nickname = `object`.myText.toString().trim { it <= ' ' }
                    }
                }
                `object`.clearMyText()
            }
            FB2Elements.HOMEPAGE -> {
                if (`object`.isDescriptionTitleInfo && `object`.isAuthor) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.titleInfo.authors.size - 1
                        `object`.fb2scheme.description.titleInfo.authors[aIndex].homepage = `object`.myText.toString().trim { it <= ' ' }
                    } else if (`object`.isTranslator) {
                        aIndex = `object`.fb2scheme.description.titleInfo.translators.size - 1
                        `object`.fb2scheme.description.titleInfo.translators[aIndex].homepage = `object`.myText.toString().trim { it <= ' ' }
                    }
                } else if (`object`.isDescriptionDocumentInfo) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.documentInfo.authors.size - 1
                        `object`.fb2scheme.description.documentInfo.authors[aIndex].homepage = `object`.myText.toString().trim { it <= ' ' }
                    }
                }
                `object`.clearMyText()
            }
            FB2Elements.EMAIL -> {
                if (`object`.isDescriptionTitleInfo && `object`.isAuthor) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.titleInfo.authors.size - 1
                        `object`.fb2scheme.description.titleInfo.authors[aIndex].email = `object`.myText.toString().trim { it <= ' ' }
                    } else if (`object`.isTranslator) {
                        aIndex = `object`.fb2scheme.description.titleInfo.translators.size - 1
                        `object`.fb2scheme.description.titleInfo.translators[aIndex].email = `object`.myText.toString().trim { it <= ' ' }
                    }
                } else if (`object`.isDescriptionDocumentInfo) {
                    if (`object`.isAuthor) {
                        aIndex = `object`.fb2scheme.description.documentInfo.authors.size - 1
                        `object`.fb2scheme.description.documentInfo.authors[aIndex].email = `object`.myText.toString().trim { it <= ' ' }
                    }
                }
                `object`.clearMyText()
            }
            ...
            FB2Elements.KEYWORDS -> {
                val keywords = `object`.myText.toString().trim { it <= ' ' }.split(",".toRegex()).toTypedArray()
                for (k in keywords) {
            ...
            FB2Elements.VERSION -> {
                if (`object`.isDescriptionDocumentInfo) `object`.fb2scheme.description.documentInfo.version = `object`.myText.toString().trim { it <= ' ' }
                `object`.clearMyText()
            }
            FB2Elements.HISTORY -> `object`.isHistory = false
            FB2Elements.BOOKNAME -> {
                if (`object`.isDescriptionPublishInfo) `object`.fb2scheme.description.publishInfo.bookname = `object`.myText.toString().trim { it <= ' ' }
                `object`.clearMyText()
            }
            FB2Elements.PUBLISHER -> {
                if (`object`.isDescriptionPublishInfo) `object`.fb2scheme.description.publishInfo.publisher = `object`.myText.toString().trim { it <= ' ' }
                `object`.clearMyText()
            }
            FB2Elements.city -> {
                if (`object`.isDescriptionPublishInfo) `object`.fb2scheme.description.publishInfo.city = `object`.myText.toString().trim { it <= ' ' }
                `object`.clearMyText()
            }
            FB2Elements.YEAR -> {
                if (`object`.isDescriptionPublishInfo) `object`.fb2scheme.description.publishInfo.year = `object`.myText.toString().trim { it <= ' ' }
                `object`.clearMyText()
            }
            FB2Elements.ISBN -> {
                if (`object`.isDescriptionPublishInfo) `object`.fb2scheme.description.publishInfo.isbn = `object`.myText.toString().trim { it <= ' ' }
                `object`.clearMyText()
            }
            else -> {
            }
